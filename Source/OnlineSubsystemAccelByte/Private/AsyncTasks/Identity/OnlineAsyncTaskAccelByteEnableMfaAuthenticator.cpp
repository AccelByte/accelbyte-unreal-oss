...
void FOnlineAsyncTaskAccelByteEnableMfaAuthenticator::Initialize()
{
	...
	if (!IsDS.IsSet())
	{
		OnlineError = ONLINE_ERROR(EOnlineErrorResult::InvalidAuth
			, FString(TEXT("enable-mfa-authenticator-failed-user-not-logged-in"))
			, FText::FromString(TEXT("Failed to enable MFA authenticator, user is not logged in!")));
		AB_OSS_ASYNC_TASK_TRACE_END_VERBOSITY(Warning, TEXT("Failed to enable MFA authenticator, user is not logged in!"));
	...
	if (IsDS.GetValue())
	{
		OnlineError = ONLINE_ERROR(EOnlineErrorResult::NotImplemented
			, FString(TEXT("enable-mfa-authenticator-not-supported-on-dedicated-server"))
			, FText::FromString(TEXT("Failed to enable MFA authenticator, operation not supported on dedicated server!")));
		AB_OSS_ASYNC_TASK_TRACE_END_VERBOSITY(Warning, TEXT("Failed to enable MFA authenticator, operation not supported on dedicated server!"));
		CompleteTask(EAccelByteAsyncTaskCompleteState::InvalidState);
		return;
	}

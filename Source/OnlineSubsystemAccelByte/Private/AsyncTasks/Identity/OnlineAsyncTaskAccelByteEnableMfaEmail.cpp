...
void FOnlineAsyncTaskAccelByteEnableMfaEmail::Initialize()
{
	...
	if (!IsDS.IsSet())
	{
		OnlineError = ONLINE_ERROR(EOnlineErrorResult::InvalidAuth
			, FString(TEXT("enable-mfa-email-failed-user-not-logged-in"))
			, FText::FromString(TEXT("Failed to enable MFA email, user is not logged in!")));
		AB_OSS_ASYNC_TASK_TRACE_END_VERBOSITY(Warning, TEXT("Failed to enable MFA email, user is not logged in!"));
	...
	if (IsDS.GetValue())
	{
		OnlineError = ONLINE_ERROR(EOnlineErrorResult::NotImplemented
			, FString(TEXT("enable-mfa-email-not-supported-on-dedicated-server"))
			, FText::FromString(TEXT("Failed to enable MFA email, operation not supported on dedicated server!")));
		AB_OSS_ASYNC_TASK_TRACE_END_VERBOSITY(Warning, TEXT("Failed to enable MFA email, operation not supported on dedicated server!"));
		CompleteTask(EAccelByteAsyncTaskCompleteState::InvalidState);
		return;
	}
	...
	User->EnableMfaEmail(Code, OnSuccessDelegate, OnErrorDelegate);

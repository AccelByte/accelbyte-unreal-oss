...
void FOnlineAsyncTaskAccelByteUpdatePassword::Initialize()
{
	...
	if (!IsDS.IsSet())
	{
		OnlineError = ONLINE_ERROR(EOnlineErrorResult::InvalidAuth
			, FString(TEXT("update-password-failed-user-not-logged-in"))
			, FText::FromString(TEXT("Failed to update password, user is not logged in!")));
		AB_OSS_ASYNC_TASK_TRACE_END_VERBOSITY(Warning, TEXT("Failed to update password, user is not logged in!"));
	...
	if (IsDS.GetValue())
	{
		OnlineError = ONLINE_ERROR(EOnlineErrorResult::NotImplemented
			, FString(TEXT("update-password-not-supported-on-dedicated-server"))
			, FText::FromString(TEXT("Failed to update password, operation not supported on dedicated server!")));
		AB_OSS_ASYNC_TASK_TRACE_END_VERBOSITY(Warning, TEXT("Failed to update password, operation not supported on dedicated server!"));
		CompleteTask(EAccelByteAsyncTaskCompleteState::InvalidState);
		return;
	}

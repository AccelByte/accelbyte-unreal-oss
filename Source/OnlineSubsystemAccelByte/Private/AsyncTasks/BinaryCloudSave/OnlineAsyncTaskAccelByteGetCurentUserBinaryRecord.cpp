...
#include "OnlineAsyncTaskAccelByteGetCurentUserBinaryRecord.h"
...
#define ONLINE_TASK_ERROR TEXT("request-failed-get-current-user-binary-record-error")

FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord(FOnlineSubsystemAccelByte* const InABInterface 
...
void FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::Initialize()
{
	Super::Initialize();

	AB_OSS_ASYNC_TASK_TRACE_BEGIN(TEXT("Get current user binary record, Key: %s"), *Key);
	...
	if (Key.IsEmpty())
	{
		...
		AB_OSS_ASYNC_TASK_TRACE_END_VERBOSITY(Warning, TEXT("Failed to get current user binary record, Key empty!"));
		return;
	}

	OnSuccessDelegate = TDelegateUtils<THandler<FAccelByteModelsUserBinaryRecord>>::CreateThreadSafeSelfPtr(this, &FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::OnSuccess);
	OnErrorDelegate = TDelegateUtils<FErrorHandler>::CreateThreadSafeSelfPtr(this, &FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::OnError);
	...
	BinaryCloudSave->GetCurrentUserBinaryRecord(Key, OnSuccessDelegate, OnErrorDelegate);

	AB_OSS_ASYNC_TASK_TRACE_END(TEXT(""));
}

void FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::Finalize()
{
	TRY_PIN_SUBSYSTEM();
...
void FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::TriggerDelegates()
{
	TRY_PIN_SUBSYSTEM();
...
void FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::OnSuccess(FAccelByteModelsUserBinaryRecord const& Result)
{
...
void FOnlineAsyncTaskAccelByteGetCurentUserBinaryRecord::OnError(int32 Code, const FString& ErrorMessage)
{
	TaskOnlineError = EOnlineErrorResult::RequestFailure;
